package com.jayaraj.hime.ui.quickblox.services;
...
import com.jayaraj.hime.ui.quickblox.fragments.AudioConversationFragment;
import com.jayaraj.hime.ui.quickblox.fragments.BaseConversationFragment;
import com.jayaraj.hime.ui.quickblox.fragments.VideoConversationFragment;
import com.jayaraj.hime.ui.quickblox.util.NetworkConnectionChecker;
import com.jayaraj.hime.ui.quickblox.utils.Consts;
import com.jayaraj.hime.ui.quickblox.utils.RingtonePlayer;
import com.jayaraj.hime.ui.quickblox.utils.SettingsUtil;
...
import static com.jayaraj.hime.ui.quickblox.utils.Consts.MAX_OPPONENTS_COUNT;
...
public class CallService extends Service {
  ...
  public void initAudioManager() {
    ...
    appRTCAudioManager.setOnWiredHeadsetStateListener(
        new AppRTCAudioManager.OnWiredHeadsetStateListener() {
          @Override
    ...
    appRTCAudioManager.setBluetoothAudioDeviceStateListener(
        new AppRTCAudioManager.BluetoothAudioDeviceStateListener() {
          @Override
    ...
    appRTCAudioManager.start(
        new AppRTCAudioManager.AudioManagerEvents() {
          ...
          public void onAudioDeviceChanged(
              AppRTCAudioManager.AudioDevice audioDevice, Set<AppRTCAudioManager.AudioDevice> set) {
            ToastUtils.shortToast("Audio Device Switched to " + audioDevice);
          }
        });

    if (currentSessionExist()
        && currentSession.getConferenceType()
    ...
    if (currentSession != null) {
      try {
        currentSession
            ...
            .setVideoCapturer(new QBRTCCameraVideoCapturer(this, null));
  ...
  public void switchCamera(CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
    if (currentSession != null && currentSession.getMediaStreamManager() != null) {
      QBRTCCameraVideoCapturer videoCapturer =
  ...
  public void switchAudio() {
    Log.v(
        TAG,
        "onSwitchAudio(), SelectedAudioDevice() = " + appRTCAudioManager.getSelectedAudioDevice());
    if (appRTCAudioManager.getSelectedAudioDevice()
  ...
  private void removeVideoTrackRenders() {
    ...
    if (!videoTrackMap.isEmpty()) {
      Iterator<Map.Entry<Integer, QBRTCVideoTrack>> entryIterator =
          videoTrackMap.entrySet().iterator();
  ...
  private class ConnectionListenerImpl extends AbstractConnectionListener {
    ...
    public void connectionClosedOnError(Exception e) {
      ...
      int reconnectHangUpTimeMillis =
          SettingsUtil.getPreferenceInt(
                  sharedPref,
  ...
  private class SessionEventsListener implements QBRTCClientSessionCallbacks {
    ...
    public void onSessionStartClose(QBRTCSession session) {
      if (session
          == WebRtcSessionManager.getInstance(getApplicationContext()).getCurrentSession()) {
    ...
    public void onReceiveHangUpFromUser(
        QBRTCSession session, Integer userID, Map<String, String> map) {
      stopRingtone();
      if (session
          == WebRtcSessionManager.getInstance(getApplicationContext()).getCurrentSession()) {
        ...
        QBUser participant =
            QbUsersDbManager.getInstance(getApplicationContext()).getUserById(userID);
        String participantName =
            participant != null ? participant.getFullName() : userID.toString();
        ToastUtils.shortToast(
            "User "
                + participantName
                + " "
                + getString(R.string.text_status_hang_up)
                + " conversation");
      }
    }
  ...
  private class SessionStateListener implements QBRTCSessionStateCallback<QBRTCSession> {
    ...
    public void onStateChanged(
        QBRTCSession qbrtcSession, BaseSession.QBRTCSessionState qbrtcSessionState) {}
  ...
  private class QBRTCSignalingListener implements QBRTCSignalingCallback {
    ...
    @Override
    public void onErrorSendingPacket(
        QBSignalingSpec.QBSignalCMD qbSignalCMD, Integer integer, QBRTCSignalException e) {
      ToastUtils.shortToast(R.string.dlg_signal_error);
    }
  }

  private class NetworkConnectionListener
      implements NetworkConnectionChecker.OnConnectivityChangedListener {
  ...
  private class VideoTrackListener implements QBRTCClientVideoTracksCallbacks<QBRTCSession> {
    ...
    public void onRemoteVideoTrackReceive(
        QBRTCSession session, QBRTCVideoTrack videoTrack, Integer userID) {
