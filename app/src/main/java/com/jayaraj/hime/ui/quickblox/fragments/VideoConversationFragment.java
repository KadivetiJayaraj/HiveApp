...
import com.jayaraj.hime.ui.quickblox.adapters.OpponentsFromCallAdapter;
...
public class VideoConversationFragment extends BaseConversationFragment
  ...
  protected void configureOutgoingScreen() {
    ...
    outgoingOpponentsRelativeLayout.setBackgroundColor(
        ContextCompat.getColor(context, R.color.grey_transparent_50));
  ...
  private void restoreSession() {
    ...
    if (!videoTrackMap.isEmpty()) {
      final Iterator<Map.Entry<Integer, QBRTCVideoTrack>> entryIterator =
      ...
      while (entryIterator.hasNext()) {
        ...
        } else if (conversationFragmentCallback.getPeerChannel(userID)
  ...
  protected void initButtonsListener() {
    ...
    cameraToggle.setOnCheckedChangeListener(
  ...
  private void toggleCameraInternal() {
    ...
    QBRTCSurfaceView surfaceViewRenderer =
        isLocalVideoFullScreen ? remoteFullScreenVideoView : localVideoView;
  ...
  private void toggleCamera(boolean isNeedEnableCam) {
    if (conversationFragmentCallback != null
        && conversationFragmentCallback.isMediaStreamManagerExist()) {
  ...
  @Override
  public void onLocalVideoTrackReceive(
      QBRTCSession qbrtcSession, final QBRTCVideoTrack videoTrack) {
  ...
  public void onRemoteVideoTrackReceive(
      QBRTCSession session, final QBRTCVideoTrack videoTrack, final Integer userID) {
    if (userID == null) {
      return;
    }
  ...
  public void OnBindLastViewHolder(
      final OpponentsFromCallAdapter.ViewHolder holder, final int position) {
  ...
  public void onItemClick(int position) {
    ...
    QBRTCTypes.QBRTCConnectionState connectionState =
        conversationFragmentCallback.getPeerChannel(userId);
    HashMap<Integer, QBRTCVideoTrack> videoTrackMap =
        conversationFragmentCallback.getVideoTrackMap();

    if (videoTrackMap != null && !videoTrackMap.containsKey(userId)
        || connectionState.ordinal()
  ...
  private void swapUsersFullscreenToPreview(int userId) {
    ...
    QBRTCVideoTrack opponentVideoTrack =
        conversationFragmentCallback.getVideoTrackMap().get(userId);

    //      get mainVideoTrack - opponent's video track from full screen
    QBRTCVideoTrack mainVideoTrack =
  ...
  private void fillVideoView(
      QBRTCSurfaceView videoView, QBRTCVideoTrack videoTrack, boolean remoteRenderer) {
