package com.jayaraj.hime.ui.quickblox.activities;
...
import com.jayaraj.hime.ui.quickblox.fragments.ConversationFragmentCallback;
import com.jayaraj.hime.ui.quickblox.fragments.IncomeCallFragment;
import com.jayaraj.hime.ui.quickblox.fragments.IncomeCallFragmentCallbackListener;
import com.jayaraj.hime.ui.quickblox.fragments.ScreenShareFragment;
...
import com.jayaraj.hime.ui.quickblox.utils.ErrorUtils;
import com.jayaraj.hime.ui.quickblox.utils.FragmentExecuotr;
...
public class CallActivity extends BaseActivity
    implements IncomeCallFragmentCallbackListener,
  ...
  private void startScreenSharing(final Intent data) {
    Fragment fragmentByTag =
        getSupportFragmentManager().findFragmentByTag(ScreenShareFragment.class.getSimpleName());
    if (!(fragmentByTag instanceof ScreenShareFragment)) {
      ...
      FragmentExecuotr.addFragment(
          ...
          screenShareFragment,
          ScreenShareFragment.class.getSimpleName());
    ...
    if (isVideoCall && checker.lacksPermissions(Consts.PERMISSIONS)) {
      if (cam) {
        ...
        ErrorUtils.showSnackbar(
            rootView,
            R.string.error_permission_video,
            R.string.dlg_allow,
  ...
  private void startLoadAbsentUsers() {
    ...
    if (!idsUsersNeedLoad.isEmpty()) {
      requestExecutor.loadUsersByIds(
          idsUsersNeedLoad,
    ...
    showIncomingCallWindowTask =
        new Runnable() {
          ...
          public void run() {
            ...
            // This is a fix to prevent call stop in case calling to user with more then one device
            // logged in.
  ...
  private void addIncomeCallFragment() {
    ...
    if (callService.currentSessionExist()) {
      ...
      FragmentExecuotr.addFragment(
          getSupportFragmentManager(), R.id.fragment_container, fragment, INCOME_CALL_FRAGMENT);
    } else {
  ...
  private void addConversationFragment(boolean isIncomingCall) {
    BaseConversationFragment conversationFragment =
        BaseConversationFragment.newInstance(
            isVideoCall ? new VideoConversationFragment() : new AudioConversationFragment(),
            isIncomingCall);
    FragmentExecuotr.addFragment(
        getSupportFragmentManager(),
        R.id.fragment_container,
        ...
        new Runnable() {
          ...
          public void run() {
            LinearLayout connectionView =
                (LinearLayout) View.inflate(CallActivity.this, R.layout.connection_popup, null);
            if (show) {
  ...
  public void onReceiveHangUpFromUser(
    ...
    if (callService.isCurrentSession(session)) {
      ...
      final String participantName =
          participant != null ? participant.getFullName() : String.valueOf(userID);
